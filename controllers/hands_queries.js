...
const getHandsNameRun = async () => {
  return pool.connect().then(async (client) => {
    const selectStatement =
    "SELECT hands.id, CONCAT(users.first_name , ' ' , users.last_name) AS full_name, hands.user_id, hands.run_id, runs.run_name, hands.hand_rank, hands.hand_number, hands.created_at ";
    const fromJoinStatement =
